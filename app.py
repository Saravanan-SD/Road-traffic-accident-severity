...
features = ['minute','hour','day_of_week','accident_cause','casualties','vehicles_involved','driver_age','vehicle_type','light_condition','lanes']
...
def main():
    with st.form('prediction_form'):
        ...
        light_condition = st.selectbox("light condition: ", options=option_light)
    ...
    if submit:
        ...
        data = np.array([minute,hour,day_of_week,accident_cause,casualties, 
                            vehicles_involved,driver_age,vehicle_type,light_condition,lanes]).reshape(1,-1)
